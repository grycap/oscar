...
func MakeJobHandler(cfg *types.Config, kubeClientset *kubernetes.Clientset, back types.ServerlessBackend, rm resourcemanager.ResourceManager) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//if mount wait until NotReady
		/*if service.Mount.Provider != "" {
			go waitPodRunning(jobUUID, kubeClientset, cfg, context.TODO())
		}*/
		c.Status(http.StatusCreated)
	}
}
...
/*
func waitPodRunning(jobUUID string, kubeClientset *kubernetes.Clientset, cfg *types.Config, ctx context.Context) error {
	opts := metav1.ListOptions{LabelSelector: "job-name=" + jobUUID}
	watcher, err := kubeClientset.CoreV1().Pods(cfg.ServicesNamespace).Watch(ctx, opts)
	...
	for {
		...
		case event := <-watcher.ResultChan():
			...
			for _, podStatus := range pod.Status.ContainerStatuses {
				if podStatus.Name == types.ContainerName {
					if pod.Status.ContainerStatuses[0].State.Terminated != nil {
						...
						request := coreClient.CoreV1().RESTClient().
							...
							VersionedParams(&v1.PodExecOptions{
								...
								Stderr:    true,
								TTY:       true,
								Container: types.Rclone_containerName,
							}, scheme.ParameterCodec)
						exec, _ := remotecommand.NewSPDYExecutor(restCfg, "POST", request.URL())
