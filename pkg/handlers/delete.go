...
func deleteBuckets(service *types.Service, cfg *types.Config, minIOAdminClient *utils.MinIOAdminClient) error {
	...
	for _, in := range service.Input {
		...
		// Delete private/shared bucket
		if err := minIOAdminClient.DeleteBucket(s3Client, splitPath[0], "all_users_group"); err != nil {
			return fmt.Errorf("unable to delete bucket %q, %v", splitPath[0], err)
		}
