...
| `propagate_token` </br> *bool*                                    | When enabled, asynchronous executions receive the caller's bearer token as the `ACCESS_TOKEN` environment variable inside the job container. Not available for synchronous invocations. Optional (default: false)                                                                                                                                |
