...
| `rewrite_target` </br> *bool* | rewrite_target is an expose boolean in the FDL (default false) that controls how OSCAR configures the NGINX Ingress/HTTProute rewrite for exposed services. If rewrite_target: false (default), ingress rewrites to /$1. If rewrite_target: true, ingress rewrites to /system/services/<service>/exposed/$1 (default: false). Optional.  |
