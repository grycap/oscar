...
class EventGatewayClient():
    ...
    def __init__(self):
        self.endpoint = utils.get_environment_variable("EVENTGATEWAY_CONFIG_ENDPOINT")
        if not self.is_http_eventype():
            self.create_http_eventype()
            
    def is_http_eventype(self):
        r = requests.get(self.endpoint + self.event_type_path)
        j = json.loads(r.text)
        if 'eventTypes' in j:
    ...
    def create_http_eventype(self):
        event_def = { "name": "http" }
        r = requests.post(self.endpoint + self.event_type_path, json=event_def)
        print(r.text)

    def register_function(self, function_name):
        func_def = { "functionId": function_name,
          "type": "http",
          "provider": { "url": "http://gateway.openfaas:8080/function/{0}".format(function_name) }
        }
        r = requests.post(self.endpoint + self.func_reg_path, json=func_def)
        print(r.text)
    ...
    def subscribe_event(self, function_name):
        ...
        r = requests.post(self.endpoint + self.event_subs_path, json=event_sub)
        print(r.text)
        j = json.loads(r.text)
