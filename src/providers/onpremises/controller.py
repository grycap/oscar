...
class OnPremises(Commands):
    ...
    def __init__(self, function_args=None):
        if function_args:
            logger.debug(f'Function creation arguments received: {function_args}')
    ...
    def update(self):
        ...
        return CustomResponse(content='Update functionality not implemented',
                              status_code=501)

    @flask_response
    def rm(self):
        # Delete minio buckets (if selected)
        if ('deleteBuckets' in self.function_args and
                self.function_args['deleteBuckets']):
    ...
    def _get_function_environment_variables(self):
        if ('envVars' not in self.function_args or
                self.function_args['envVars']):
            if 'name' in self.function_args:
                deploy_envvars = self.kubernetes.get_deployment_envvars(
                    self.function_args['name'],
                    'openfaas-fn')
                for envvar in deploy_envvars:
                    if 'name' in envvar and 'value' in envvar:
                        self.add_function_environment_variable(envvar['name'],
                                                               envvar['value'])
    ...
    def _set_io_folder_variables(self, provider_id):
        self.add_function_environment_variable(
            ...
            self.minio.get_input_bucket_name())
        self.add_function_environment_variable(
            f'STORAGE_PATH_OUTPUT_{provider_id}',
            self.minio.get_output_bucket_name())
    ...
    def _parse_output(self, response):
        if response:
            if response.status_code == 200:
                ...
                logger.info(f'Request call returned code "{response.status_code}": {response.text}')
