...
class MinioClient():
    ...
    def __init__(self, function_args, minio_id):
        ...
        if minio_id and 'envVars' in function_args:
            if 'STORAGE_PATH_OUTPUT_{}'.format(minio_id) in function_args['envVars']:
                self.output_bucket = function_args['envVars'][f'STORAGE_PATH_OUTPUT_{minio_id}']
    ...
    def create_input_bucket(self):
        self._create_bucket(f'{self.function_name}-in')
        self._set_bucket_event_notification(f'{self.function_name}-in')

    def create_output_bucket(self):
        if not hasattr(self, 'output_bucket'):
            self._create_bucket(f'{self.function_name}-out')
    ...
    def delete_input_bucket(self):
        input_bucket_name = self.get_input_bucket_name()
        self._delete_bucket_event_notification(input_bucket_name)
        self._delete_bucket(input_bucket_name)

    def delete_output_bucket(self):
        self._delete_bucket(self.get_output_bucket_name())
    ...
    def get_input_bucket_name(self):
        ...
        return f'{self.function_name}-in'
