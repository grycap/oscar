...
# OpenClaw >= 2026.3.x requires explicit Control UI origin handling when binding
# on non-loopback addresses. For OSCAR exposed services, trust host-header origin
# fallback by default unless explicit allowed origins are provided.
if [[ -n "${OPENCLAW_GATEWAY_ALLOWED_ORIGINS:-}" ]]; then
  if ! node /app/openclaw.mjs config set gateway.controlUi.allowedOrigins "${OPENCLAW_GATEWAY_ALLOWED_ORIGINS}" --json >/dev/null 2>&1; then
    echo "warning: could not set gateway.controlUi.allowedOrigins from OPENCLAW_GATEWAY_ALLOWED_ORIGINS" >&2
  fi
else
  if ! node /app/openclaw.mjs config set gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback true --json >/dev/null 2>&1; then
    echo "warning: could not set gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true" >&2
  fi
fi
